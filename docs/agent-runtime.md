...
The mailbox itself is now a little more explicit too. Session-lane messages carry a stable message id plus completion/result state as their own object rather than being only a transient deque entry, and lane draining now targets a specific enqueued message instead of relying on a more implicit "push then empty the whole deque" shape. That is still synchronous today, but it is a cleaner seam for later waitable mailbox messages, per-lane scheduling, or external actor-style wakeups.
