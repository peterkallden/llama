...
That seam now also has a small event-emitter shape rather than only ad hoc `push_back(...)` or four-argument sink calls. The daemon event sink now takes a full event object, and session/service code can stamp request, turn, session, namespace, project, and optional operation context through a small `common_agent_event_emitter` before emitting typed events. The current implementation intentionally keeps the external JSONL/admin contract stable, but internally it gives the host one clearer append path for lane/session/service events and a better place to centralize sequencing and later fan-out without forcing transport logic back into runtime code.
...
That bounded daemon host slice is now in place as well: daemon client/admin helpers, JSONL protocol shaping, lifecycle/event/collector code, dispatcher/service/runtime assembly, and the daemon entrypoint now live under `tools/agent/daemon`, again with compatibility headers left in the old path for a gradual migration.
