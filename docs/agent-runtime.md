...
That manager has now also taken one small step toward an actor-like session shape. Each keyed session lane now owns its resident host plus a small turn-state record: queued-turn count, any currently active request/turn id plus phase, and the most recent completed/failed/cancelled turn phase for diagnostics. Execution is still synchronous once a turn enters the lane, but the lane contract is now explicit enough that later async scheduling can add waiting states and mailbox ownership without moving session bookkeeping back up into the daemon transport or dispatcher.
...
Session status is now slightly richer too. In addition to key, project, scope, and policy-pack identity, the daemon-side session descriptor can report queued-turn count plus active request/turn and last-turn phase details. The current JSONL admin path still uses this only as bounded diagnostics, but it gives later actor/scheduler work a stable place to expose per-session execution state without inventing a second status model beside the session manager itself.
...
- `llama-agent-runtime-session-manager-smoke`, verifying the new per-session lane bookkeeping for failed turns, host-owned cancellation, active-turn cancel routing, reset, and close without needing a live model
