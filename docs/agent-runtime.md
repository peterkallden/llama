...
That manager has now also taken one small step toward an actor-like session shape. Each keyed session lane now owns its resident host plus a small turn-state record: queued-turn count, any currently active request/turn id plus phase, and the most recent completed/failed/cancelled turn phase plus disposition for diagnostics. The lane now also has a real internal mailbox plus a named turn-disposition seam, and lane draining advances the active turn through explicit internal phases rather than treating the whole lane step as one opaque call. Processing still drains synchronously today, but the shape is now explicit enough that later async scheduling can introduce waiting states, multi-step advancement, and mailbox ownership without moving session bookkeeping back up into the daemon transport or dispatcher.
