...
The first feedback-aware slice also supplies native `tool_failure` signals to the post-turn candidate extractor when a registered plan-step tool fails and its capped error observation is recorded. If a plan later reaches `completed` after such a failure, the runtime adds a `successful_recovery` signal that points to the same evidence. Signals are evidence and metadata only: the model may cite their observation IDs for a reusable procedure candidate, but they never write memory directly. Accepted memories retain their signal types in `learning_signal_types` metadata. Explicit user corrections and reflection learning hints remain follow-up interfaces because they need an equally explicit turn/provenance boundary.
