...
The request/result contract is also intentionally narrower than full CLI state now: agent generation requests carry only purpose, trace metadata, optional agent scope metadata, messages, tools, schema, and a small generation options struct instead of the whole top-level CLI `args`. That options struct is now generation-shaped rather than chat-turn-shaped, so later resident backends can honor bounded prompt or predict time budgets without reopening the CLI/runtime coupling. The local CLI-backed path now also fills the same generation result envelope and honors those bounded generation options, instead of treating local inference as a special `bool + output` case. That keeps resident hosting as an implementation detail of the inference backend instead of a new agent runtime architecture.
