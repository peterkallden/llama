...
The separate agent bootstrap is also idempotent, but installs curated procedure memories and blueprint plans. It now has a common logical JSON parser and serializer (`schema_version`, name, version, procedures and blueprints), used for external package files and regression coverage. The compiled default remains a small local fallback; `pocs/memory/bootstrap/default-v1.json` carries the equivalent starter definitions and can be loaded explicitly with `--agent-bootstrap-file PATH`. The current package form contains stable logical definitions only: it serializes neither persisted IDs, embeddings, task-plan state, observations nor event history. General export/import of the wider memory store is deliberately the next slice, rather than being implied by this bootstrap-file parser.
...
Blueprint selection can be explicit with `--agent-blueprint repository-change --plan-id change-42`, or use `--agent-blueprint auto --plan-id change-42`. The selector contract and native orchestration live in `common/agent`, while the llama-backed JSON adapter remains CLI-owned. Candidates are derived from the installed package definitions and carry only logical ID, persisted ID and a short description. Selection is capped at 16 candidates, requires confidence of at least `0.75`, and otherwise falls back safely to ordinary plan creation. A supplied/resumed plan always takes precedence and skips model selection; the selector cannot choose scope, identity, task-plan ID, an arbitrary plan, or an uninstalled blueprint.
