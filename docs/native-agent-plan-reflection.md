...
Reflection uses a compact outer JSON schema to keep local grammar sampling bounded. Its `operations` entries are then parsed and validated natively against the supported operation shapes before they can mutate a plan; a permissive model-facing object therefore never grants a permissive runtime mutation path.
