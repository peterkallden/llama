...
When built with `LLAMA_MEMORY=ON` and `LLAMA_AGENT_REFLECTION=ON`, `llama-agent run` exposes this path with `--tool-profile minimal|memory-read|memory|research`. It bootstraps the selected profile in process, binds memory scope and the optional embedding provider from CLI-owned runtime state, performs bounded registered-tool rounds, and completes with a tool-free final generation. `memory_remember` is available as a policy-gated proposal in `memory` and `research`; it is not an unrestricted write capability. `llama-memory chat` still accepts the older direct memory-tool flags on its compatibility path, but `llama-agent` now exposes tools through `--tool-profile`.

### Detailed overrides

`--agent-profile` is a convenience layer. An explicitly supplied `--tool-profile`, `--planning-mode`, `--reflection-mode` or `--memory-learn` overrides only that dimension of the chosen profile. This is useful for experiments such as `--agent-profile research --reflection-mode off`.
...
As a preparatory refactor, the llama-backed agent path now routes planner, blueprint selection and binding, reasoning, draft synthesis, reflection, and post-turn memory learning through a small `common_agent_inference` abstraction. The local CLI implementation still uses the existing `generate_chat_turn(...)` path, while the resident path can submit a single non-streaming completion through an in-process `server_context`. The immediate goal remains an agent inference abstraction before any daemon, socket, or transport lifecycle work.

The request/result contract is intentionally narrower than full CLI state: agent generation requests carry only purpose, trace metadata, optional agent scope metadata, messages, tools, schema, and a small generation options struct instead of the top-level CLI `args`. That options struct is generation-shaped rather than chat-turn-shaped, so both local and resident backends can honor bounded prompt or predict time budgets without reopening CLI/runtime coupling. Both implementations fill the same generation result envelope, which keeps resident hosting as an implementation detail of the inference backend.

The CLI mini-agent runtime chooses its inference backend through a small factory, with `cli` still the default and `server-context` available as an explicit opt-in for local resident-host smoke testing. The runtime path also now carries small contracts for inference options, runtime generation and memory-learning configuration, orchestration policy, and active plan identity. CLI `args` are translated into those contracts at the edge; planner auto-selection, blueprint selection and binding, export/bootstrap, and runtime request construction no longer treat `args` as their operating state.

The current runtime host layer is still in-process, synchronous, and deliberately boring: it owns turn execution over prepared host inputs, session initialization, backend selection, completion hooks and session reset, while the CLI-specific host adapter owns argument-derived wiring such as episode recording, result printing and chat/mini host-input construction. Tool execution remains synchronous through the existing registry/provider path. This keeps planning, memory learning, reflection and tool policy semantics unchanged while moving CLI knowledge out of the host core.

The next runtime-oriented documentation step should be a dedicated agent-runtime note rather than continuing to grow this planning/reflection document. That file should describe the runtime driver boundary, resident host ownership, cancellation and timeout policy, and eventual multi-turn lifecycle once those pieces move beyond the current in-process smoke path.
