...
Those direct memory-tool flags remain part of the `llama-memory chat` compatibility path. `llama-agent run` now uses the catalog/provider path instead: choose a catalog-backed profile such as `memory-read`, `memory`, or `research` when agent turns should expose memory tools.
...
The profile is bootstrapped in process. `memory-read` exposes registered read-only executors: `calculator`, UTC `time_now`, `memory_search`, and `memory_get`; `plan_get` becomes available later when a plan store is bound. `memory` and `research` additionally expose `memory_remember` as a policy-gated proposal: it invokes the existing native decision and audit path, never a generic write API. `research` also exposes bounded repository inspection plus native `web_search` (DuckDuckGo Lite) and `web_fetch` (public HTTPS text extraction); neither provides shell, arbitrary HTTP method, upload or generic curl access. On Windows those two tools use WinHTTP and the system certificate store; elsewhere they use the configured httplib/OpenSSL-compatible TLS transport. Native tool handlers now return a structured execution result with stable `failure_code`, `failure_class`, `retryable`, and `safe_summary` fields instead of relying on free-form error text. `--max-tool-rounds` defaults to one and permits up to four sequential one-call rounds. After the final permitted result, the answer turn has tools disabled. In `llama-agent`, `--tool-profile` is now the supported path for tool exposure. Persistent Cozo catalog storage is a later slice.
...
- Compact mini-plan JSON is grammar-constrained and parser/policy-validated. The model-facing schema is narrower than the parser so local grammar sampling stays simple, while native parsing still accepts safe shorthand for older/imported compact proposals. The scheduler executes a bounded dependency-ready chain; it remains sequential and deliberately excludes arbitrary shell or write execution.
