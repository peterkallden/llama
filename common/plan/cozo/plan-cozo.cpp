...
static json serialize(const common_plan_state & p) { json steps=json::array(), observations=json::array(), constraints=json::array(), assumptions=json::array(); for(const auto & s:p.steps) { json step={{"id",s.id},{"title",s.title},{"objective",s.objective},{"status",step_status_name(s.status)},{"depends_on",s.depends_on},{"blocked_by",s.blocked_by},{"required_evidence",s.required_evidence},{"source_memory_ids",s.source_memory_ids},{"optional",s.optional},{"generated_from_memory",s.generated_from_memory},{"created_at",s.created_at},{"updated_at",s.updated_at}}; if(s.selected_tool)step["selected_tool"]=*s.selected_tool; if(s.result_summary)step["result_summary"]=*s.result_summary; if(s.tool_call)step["tool_call"]={{"name",s.tool_call->name},{"arguments_json",s.tool_call->arguments_json}}; steps.push_back(std::move(step)); } for(const auto & observation:p.observations) observations.push_back({{"id",observation.id},{"source",observation.source},{"summary",observation.summary},{"confidence",observation.confidence},{"evidence_ids",observation.evidence_ids},{"created_at",observation.created_at}}); for(const auto & constraint:p.constraints) constraints.push_back({{"id",constraint.id},{"description",constraint.description},{"hard",constraint.hard}}); for(const auto & assumption:p.assumptions) assumptions.push_back({{"id",assumption.id},{"statement",assumption.statement},{"confidence",assumption.confidence},{"valid",assumption.valid},{"evidence_ids",assumption.evidence_ids}}); return {{"id",p.id},{"session_id",p.session_id},{"kind",kind_name(p.kind)},{"derived_from_plan_id",p.derived_from_plan_id},{"scope",scope_name(p.scope)},{"status",status_name(p.status)},{"goal",p.goal},{"success_criteria",p.success_criteria},{"steps",steps},{"observations",observations},{"constraints",constraints},{"assumptions",assumptions},{"active_step_id",p.active_step_id},{"next_action",p.next_action},{"version",p.version},{"created_at",p.created_at},{"updated_at",p.updated_at}}; }
