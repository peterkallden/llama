...
bool common_plan_cozo_store::create(const common_plan_state&p,std::string&error){return cache.create(p,error)&&persist_plan(p,error);}
std::optional<common_plan_state> common_plan_cozo_store::get(const std::string&id,std::string&error){return cache.get(id,error);}
bool common_plan_cozo_store::apply(const common_plan_operation&o,common_plan_state&updated,std::string&error){if(!cache.apply(o,updated,error))return false; if(!persist_plan(updated,error))return false;auto events=cache.history(o.plan_id,error);const auto &e=events.back();std::string out;json rows=json::array({json::array({o.plan_id,e.sequence,json({{"accepted",e.accepted},{"prior_version",e.prior_version},{"new_version",e.new_version},{"operation",common_plan_operation_kind_name(o.kind)},{"reason_summary",e.reason_summary},{"evidence_ids",o.evidence_ids}}).dump()})});return run("?[plan_id, sequence, event_json] <- $rows :put agent_plan_event { plan_id, sequence => event_json }",json({{"rows",rows}}).dump(),out,error);}
