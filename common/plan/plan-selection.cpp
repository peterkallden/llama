...
bool common_plan_scope_matches(const common_plan_state & plan, common_plan_scope scope,
    ...
    switch (scope) {
        case common_plan_scope::turn: return !turn_id.empty() && plan.turn_id == turn_id;
        case common_plan_scope::session: return !session_id.empty() && plan.session_id == session_id;
