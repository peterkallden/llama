...
std::vector<common_tool_definition> builtin_definitions() {
    ...
    return {
        ...
        tool("dataset.inspect", "Inspect bounded metadata for a host-owned dataset reference or legacy file.", R"({"type":"object","additionalProperties":false,"properties":{"dataset":{"type":"string","minLength":1,"maxLength":512},"path":{"type":"string","minLength":1,"maxLength":512}}})", object, "builtin.dataset.inspect", common_tool_risk_class::local_read),
        tool("dataset.schema", "Return a bounded typed schema for a dataset reference or legacy CSV file.", R"({"type":"object","additionalProperties":false,"properties":{"dataset":{"type":"string","minLength":1,"maxLength":512},"path":{"type":"string","minLength":1,"maxLength":512},"max_rows":{"type":"integer","minimum":1,"maximum":10000}}})", object, "builtin.dataset.schema", common_tool_risk_class::local_read),
        tool("dataset.sample", "Return a bounded sample from a dataset reference or legacy CSV file.", R"({"type":"object","additionalProperties":false,"properties":{"dataset":{"type":"string","minLength":1,"maxLength":512},"path":{"type":"string","minLength":1,"maxLength":512},"rows":{"type":"integer","minimum":1,"maximum":100}}})", object, "builtin.dataset.sample", common_tool_risk_class::local_read),
        tool("document.tables", "List bounded tables exposed by a host-owned document representation. Names are model-friendly aliases; node IDs and indexes are stable host addresses.", R"({"type":"object","additionalProperties":false,"required":["resource"],"properties":{"resource":{"type":"string","minLength":1,"maxLength":512},"max_results":{"type":"integer","minimum":1,"maximum":64}}})", R"({"type":"object","additionalProperties":false,"required":["resource","tables","truncated"],"properties":{"resource":{"type":"string","x-agent-type":"resource_ref"},"tables":{"type":"array","maxItems":64,"items":{"type":"object","additionalProperties":false,"required":["index","name","node_id"],"properties":{"index":{"type":"integer","minimum":0},"name":{"type":"string","maxLength":256},"caption":{"type":"string","maxLength":512},"node_id":{"type":"string","maxLength":512}}}},"truncated":{"type":"boolean"}}})", "builtin.document.tables", common_tool_risk_class::local_read),
        tool("document.table", "Resolve one document table by a unique name, index or node ID and return its bounded dataset handle.", R"({"type":"object","additionalProperties":false,"required":["resource"],"properties":{"resource":{"type":"string","minLength":1,"maxLength":512},"table":{"type":"string","minLength":1,"maxLength":256},"table_index":{"type":"integer","minimum":0,"maximum":1000000},"node_id":{"type":"string","minLength":1,"maxLength":512}},"oneOf":[{"required":["table"]},{"required":["table_index"]},{"required":["node_id"]}]})", R"({"type":"object","additionalProperties":false,"required":["table_index","name","node_id","dataset","source_resource"],"properties":{"table_index":{"type":"integer","minimum":0},"name":{"type":"string","maxLength":256},"node_id":{"type":"string","maxLength":512},"dataset":{"type":"string","x-agent-type":"dataset_ref","minLength":1,"maxLength":512},"source_resource":{"type":"string","x-agent-type":"resource_ref","minLength":1,"maxLength":512}}})", "builtin.document.table", common_tool_risk_class::local_read),
...
std::vector<common_tool_profile> builtin_profiles() {
    return {
        ...
        policy_profile(profile("research", "Memory profile with repository, resource and safe network-read declarations.", {"calculator", "time_now", "memory_search", "memory_get", "memory_inspect", "memory_conflict_check", "memory_remember", "plan_get", "plan_propose", "repository.search", "repository.read", "repository.list", "repository.diff", "resource_inspect", "resource_read", "document.tables", "document.table", "web_search", "web_fetch"}), true, true),
