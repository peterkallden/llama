...
std::vector<common_tool_definition> builtin_definitions() {
    ...
    return {
        ...
        tool("memory_remember", "Propose a policy-gated persistent memory.", R"({"type":"object","additionalProperties":false,"required":["kind","content"],"properties":{"kind":{"type":"string","enum":["fact","preference","procedure","goal","observation","reflection","episode"]},"content":{"type":"string","minLength":1,"maxLength":512},"importance":{"type":"number","minimum":0,"maximum":1},"confidence":{"type":"number","minimum":0,"maximum":1},"rationale":{"type":"string","minLength":1,"maxLength":240}}})", object, "builtin.memory_remember", common_tool_risk_class::memory_proposal, true),
        ...
        tool("mock_web_search", "Return deterministic search results for end-to-end tests.", R"({"type":"object","additionalProperties":false,"required":["query"],"properties":{"query":{"type":"string","minLength":1,"maxLength":1024},"limit":{"type":"integer","minimum":1,"maximum":5}}})", object, "builtin.mock_web_search", common_tool_risk_class::network_read),
...
std::vector<common_tool_profile> builtin_profiles() {
    return {
        ...
        profile("memory-read", "Read-only scoped memory and plan inspection.", {"calculator", "time_now", "memory_search", "memory_get", "memory_inspect", "memory_conflict_check", "plan_get"}),
        profile("memory", "Memory inspection plus policy-gated memory and plan proposals.", {"calculator", "time_now", "memory_search", "memory_get", "memory_inspect", "memory_conflict_check", "memory_remember", "memory_propose_update", "memory_propose_forget", "memory_link", "memory_compact_propose", "plan_get", "plan_propose"}),
        profile("research", "Memory profile with repository inspection and safe network-read declarations.", {"calculator", "time_now", "memory_search", "memory_get", "memory_inspect", "memory_conflict_check", "memory_remember", "plan_get", "plan_propose", "repository_list", "repository_search", "repository_read", "repository_diff", "repository_log", "mock_web_search", "web_fetch"}),
    };
}
