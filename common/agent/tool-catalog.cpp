...
std::string canonical_tool_name(const std::string & name) {
    if (name == "repository_diff") return "repository.diff";
    if (name == "workspace_list") return "workspace.list";
    if (name == "workspace_read") return "workspace.read";
    if (name == "workspace_search") return "workspace.search";
    if (name == "build_target") return "development.build";
    if (name == "test_run") return "development.test";
    return name;
}
...
std::vector<common_tool_definition> builtin_definitions() {
    ...
    return {
        ...
        tool("repository.diff", "Return a bounded read-only Git working-tree diff summary.", R"({"type":"object","additionalProperties":false})", object, "builtin.repository_diff", common_tool_risk_class::local_read),
...
std::vector<common_tool_profile> builtin_profiles() {
    return {
        ...
        policy_profile(profile("all-configured", "All catalogued tools, still subject to host policy and confirmation.", {"calculator", "time_now", "memory_search", "memory_get", "memory_inspect", "memory_conflict_check", "memory_remember", "memory_propose_update", "memory_propose_forget", "memory_link", "memory_compact_propose", "plan_get", "plan_propose", "repository_list", "repository_search", "repository_read", "repository.diff", "repository_log", "repository_status", "repository_changed_files", "workspace.list", "workspace.read", "workspace.search", "development.build", "development.test", "resource_read", "web_search", "web_fetch"}), true, true),
    };
}
...
const common_tool_definition * common_tool_catalog::find_definition(const std::string & name, uint32_t version) const {
    const auto it = definitions.find(canonical_tool_name(name) + "@" + std::to_string(version));
