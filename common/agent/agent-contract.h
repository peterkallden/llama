...
enum class common_agent_event_type {
    ...
    memory_remembered,
    memory_rejected,
    // Plan and reflection event types belong to the plan/reflection branch.
    // plan_created,
    // plan_updated,
...
struct common_agent_request {
    ...
    // Planning scope is deliberately independent from memory scope. In
    // particular, a global plan must not grant global-memory access.
...
struct common_agent_result {
    ...
    // Plan/reflection events are added with the plan/reflection implementation.
