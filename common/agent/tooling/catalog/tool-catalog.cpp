...
std::vector<common_tool_definition> builtin_definitions() {
    ...
    std::vector<common_tool_definition> definitions = {
        ...
        tool("memory_search", "Search memories available to the current runtime scope.", R"({"type":"object","additionalProperties":false,"required":["query"],"properties":{"query":{"type":"string","minLength":1,"maxLength":1024},"limit":{"type":"integer","minimum":1,"maximum":8}}})", object, "builtin.memory_search", common_tool_risk_class::local_read, false, 30000),
...
std::vector<common_tool_profile> builtin_profiles() {
    return {
        ...
        policy_profile(profile("developer-read", "Read-only workspace, repository and resource inspection tools.", {"calculator", "time_now", "repository.list", "repository.search", "repository.read", "repository.diff", "repository.log", "repository.status", "repository.changed_files", "workspace.list", "workspace.read", "workspace.search", "diagnostics.symbol", "diagnostics.references", "diagnostics.call_hierarchy", "diagnostics.native_crash", "resource_inspect", "resource_read", "document.tables", "document.table"}), false, false),
