...
agent_smoke_run_logged "$log_path" "$agent_bin" run --model "$model" --agent-profile static --agent-bootstrap none --agent-inference-backend cli --agent-trace --n-predict "${LLAMA_AGENT_N_PREDICT:-96}" --threads "${LLAMA_AGENT_THREADS:-4}" -ngl 0 --prompt "$prompt"
