...
$token = "tcp-smoke-token-$([guid]::NewGuid().ToString('N'))"
$env:LLAMA_AGENT_TCP_TOKEN = $token
$process = $null
$client = $null
$writer = $null
$reader = $null
try {
    ...
    while (-not $client.Connected -and [DateTime]::UtcNow -lt $deadline) {
        try { $client.ConnectAsync("127.0.0.1", $Port).Wait(250) } catch { }
    ...
    $writer.WriteLine((@{ authorization = "Bearer $token" } | ConvertTo-Json -Compress))
    ...
    if (-not $shutdown.ok -or $shutdown.event -ne "shutdown") { throw "TCP shutdown failed" }
