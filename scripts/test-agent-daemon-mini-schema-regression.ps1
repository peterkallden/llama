...
$requests = @(
    '{"mode":"mini","prompt":"Say OK after making a tiny plan.","session_id":"mini-schema-regression","namespace_id":"mini-schema","project_id":"repo-smoke","memory_scope":"project","plan_scope":"project"}',
    '{"command":"shutdown"}'
)

$runId = [guid]::NewGuid().ToString("N")
$requestsPath = Join-Path $env:TEMP "llama-agent-daemon-mini-schema-regression-$runId-requests.txt"
$stdoutPath = Join-Path $env:TEMP "llama-agent-daemon-mini-schema-regression-$runId-stdout.log"
$stderrPath = Join-Path $env:TEMP "llama-agent-daemon-mini-schema-regression-$runId-stderr.log"
...
try {
    ...
    if (-not $turn.ok) {
        Show-Diagnostics -Path $stderrPath
        throw "Mini schema regression turn failed: $($lines[1])"
    }
    if ($turn.response -ne "OK") {
        Show-Diagnostics -Path $stderrPath
        throw "Mini schema regression response mismatch: $($lines[1])"
    }
    if (-not $turn.plan_id) {
        Show-Diagnostics -Path $stderrPath
        throw "Mini schema regression response did not include plan_id: $($lines[1])"
    }
    if ($turn.event_count -lt 1) {
        Show-Diagnostics -Path $stderrPath
        throw "Mini schema regression did not report runtime events: $($lines[1])"
    }
    ...
    Write-Host ""
    Write-Host "Agent daemon mini schema regression test complete."
    Write-Host $lines[0]
    Write-Host $lines[1]
    Write-Host $lines[2]
}
