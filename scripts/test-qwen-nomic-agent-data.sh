...
prompt='Perform a bounded data-analysis task using the two CSV datasets in the workspace. Discover and inspect them, join orders with customers on customer_id, aggregate the joined rows by segment using sum(amount), and use statistics.describe on amount. Report the exact total 40 and mention the tools/results used. Do not guess; use the data tools.'
agent_smoke_run_logged "$agent_log" "$agent_bin" run --backend cozo --memory-db "$work_dir/memory.cozo" --plan-backend cozo --plan-db "$work_dir/plan.cozo" --data-backend cozo --data-db "$data_db" --model "$model" --embedding-model "$embedding_model" --agent-profile research --tool-profile all-configured --thinking-mode "${LLAMA_AGENT_THINKING_MODE:-reflective}" --max-reflection-rounds 1 --max-research-iterations 1 --agent-plan auto --max-plan-revisions 2 --repository-root "$work_dir" --max-tool-rounds 4 --memory-project qwen-nomic-data --plan-scope project --agent-trace --plan-show-summary --prompt "$prompt" --n-predict "${LLAMA_AGENT_N_PREDICT:-96}" --threads "${LLAMA_AGENT_THREADS:-2}" -ngl 0
grep -Fq 'data.join' "$agent_log"; grep -Fq 'data.aggregate' "$agent_log"; grep -Fq '40' "$agent_log"
