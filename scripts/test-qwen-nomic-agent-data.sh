...
agent_smoke_run_logged "$agent_log" "$agent_bin" run --backend cozo --memory-db "$work_dir/memory.cozo" --plan-backend cozo --plan-db "$work_dir/plan.cozo" --data-backend cozo --data-db "$data_db" --model "$model" --embedding-model "$embedding_model" --agent-profile research --tool-profile analysis --thinking-mode "${LLAMA_AGENT_THINKING_MODE:-reflective}" --max-reflection-rounds 1 --max-research-iterations 1 --agent-plan auto --max-plan-revisions 2 --repository-root "$work_dir" --max-tool-rounds 16 --memory-project qwen-nomic-data --plan-scope project --agent-trace --generation-trace --require-tool-execution --plan-show-summary --prompt "$prompt" --n-predict "${LLAMA_AGENT_N_PREDICT:-96}" --context-size "${LLAMA_AGENT_CONTEXT_SIZE:-3072}" --threads "${LLAMA_AGENT_THREADS:-2}" -ngl 0
