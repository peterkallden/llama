...
for ((attempt = 1; attempt <= attempts; ++attempt)); do
    if adb shell cmd package list packages >/dev/null 2>&1 &&
        ...
        echo "Android Package Manager and Activity Manager are ready after attempt ${attempt}/${attempts}."
        exit 0
    fi
    sleep 2
done

echo "Android Package Manager and Activity Manager did not become ready after ${attempts} attempts." >&2
