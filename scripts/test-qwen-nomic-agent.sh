...
agent_smoke_run_logged "$log_path" "$agent_bin" run --backend in-memory --plan-backend in-memory --model "$model" --embedding-model "$embedding_model" --agent-profile static --agent-bootstrap none --agent-plan off --agent-blueprint off --thinking-mode "${LLAMA_AGENT_THINKING_MODE:-reflective}" --agent-inference-backend cli --agent-trace --n-predict "${LLAMA_AGENT_N_PREDICT:-16}" --threads "${LLAMA_AGENT_THREADS:-4}" -ngl 0 --prompt "${LLAMA_AGENT_PROMPT:-Say OK after making a tiny plan. Reply with OK only.}"
