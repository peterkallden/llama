...
$cmake = Resolve-CMake
$exePath = Join-Path $repoRoot "$BuildDir\bin\Release\llama-agent-daemon-dispatcher-smoke.exe"
...
if ($Build) {
    & $cmake --build $BuildDir --config Release --target llama-agent-daemon-dispatcher-smoke -j 1
