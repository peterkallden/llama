...
make_agent_learning_lifecycle_store(
    ...
    if (!parse_common_learning_transaction_backend(requested_backend.empty() ? "auto" : requested_backend, backend)) {
        error = "unknown adaptation lifecycle backend: " + requested_backend;
        return nullptr;
    }
    if (backend == common_learning_transaction_backend::automatic) {
        if (path.empty()) backend = common_learning_transaction_backend::in_memory;
            ...
            error = "no persistent adaptation lifecycle backend is compiled in";
            return nullptr;
        }
#endif
    }
    if (backend == common_learning_transaction_backend::in_memory) {
        if (!path.empty()) { error = "in-memory adaptation lifecycle backend does not accept a path"; return nullptr; }
        return std::make_unique<common_learning_in_memory_lifecycle_store>();
    }
    if (backend == common_learning_transaction_backend::jsonl) {
        auto store = std::make_unique<common_learning_jsonl_lifecycle_store>();
        if (!store->open(path, error)) return nullptr;
        return store;
    }
    if (path.empty()) {
        error = "persistent adaptation lifecycle backend requires a path";
        return nullptr;
    }
    if (backend == common_learning_transaction_backend::cozo) {
#ifdef LLAMA_AGENT_ADAPTATION_USE_COZO
        auto store = std::make_unique<common_agent_cozo_learning_lifecycle_store>();
        if (!store->open(path, error)) return nullptr;
        return store;
#else
        error = "adaptation lifecycle Cozo backend requires a build with LLAMA_MEMORY_COZO";
        return nullptr;
#endif
    }
    if (backend == common_learning_transaction_backend::sqlite) {
#ifdef LLAMA_AGENT_ADAPTATION_USE_SQLITE
        auto store = std::make_unique<common_agent_sqlite_learning_lifecycle_store>();
        if (!store->open(path, error)) return nullptr;
        return store;
#else
        error = "adaptation lifecycle SQLite backend requires a build with LLAMA_AGENT_STORAGE_SQLITE";
        return nullptr;
#endif
    }
    error = "unsupported adaptation lifecycle backend";
    return nullptr;
}
