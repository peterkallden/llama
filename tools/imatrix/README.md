...
The quantizer also has an experimental `--quant-repair` pass. It treats a selected tensor type as a repair-before-promote candidate, evaluates cheaper compatible types against the same imatrix-weighted samples, and accepts them only when their measured error is close to the selected type or below a conservative absolute threshold. By default it enables `clipping,gain,scale`; `--quant-repair-methods` can be used to select an exact subset.
