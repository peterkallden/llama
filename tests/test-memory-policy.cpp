...
int main() {
    ...
    auto conflict = common_memory_evaluate_remember_request(
        store, request_for("The project codename is Atlas."), {0.96f, 0.04f}, 1236, error);
    assert(error.empty());
    ...
    auto allowed_preference = common_memory_evaluate_remember_request(
        store, request_for("Peter prefers short commit messages.", common_memory_kind::preference), {0.05f, 0.95f}, 1241, error);
    assert(error.empty());
