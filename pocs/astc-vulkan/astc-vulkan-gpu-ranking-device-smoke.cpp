...
std::array<uint8_t, 16> constant_astc(uint16_t value) {
    ...
    for (uint32_t channel = 0; channel < 4; ++channel) {
        block[8 + channel * 2] = static_cast<uint8_t>(value & 0xffu);
        block[9 + channel * 2] = static_cast<uint8_t>(value >> 8u);
    }
    return block;
}
...
int main(int argc, char ** argv) {
    ...
    const VkApplicationInfo application{VK_STRUCTURE_TYPE_APPLICATION_INFO, nullptr,
        "astc-vulkan-gpu-ranking-device-smoke", 1, "llama.cpp", 1, VK_API_VERSION_1_0};
    ...
    const astc_vulkan_gpu_ranking_candidate neutral{constant_astc(0x8000), astc_vulkan_paired_layout::rg_b};
    const astc_vulkan_gpu_ranking_candidate alternate{constant_astc(0xc000), astc_vulkan_paired_layout::rg_b};
    const std::vector<std::vector<astc_vulkan_gpu_ranking_candidate>> pools{{neutral, alternate}};
    ...
    const bool initialized = packed && session.init(physical, device, queue, family, atlas, 1, 8, 10, 2, spirv, error);
    ...
    if (!ran || deltas.size() != 40) { std::fprintf(stderr, "GPU ranking smoke failed: %s\n", error.c_str()); return 1; }
    const float decoded_delta = static_cast<float>(0xc000u - 0x8000u) / 65535.0f;
    for (uint32_t candidate = 0; candidate < 2; ++candidate) for (uint32_t sample = 0; sample < 2; ++sample) {
        const float expected = candidate == 0 ? 0.0f : decoded_delta * (sample == 0 ? 36.0f : 4.0f);
