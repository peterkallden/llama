...
int main(int argc, char ** argv) {
    ...
    if (command == "compiled-inspect" || command == "compile-inspect") {
        ...
        std::printf("astc-cache compiled-inspect path=%s version=%u fingerprint=%s bootstrap-gguf-bytes=%zu "
                    ...
                    compiled.native_table.size(), compiled.native_payload.size());
        if (!compiled.embedding_metadata.empty()) {
            std::printf("astc-cache compiled-inspect embedding=e1-local-10x5 metadata-bytes=%zu payload-bytes=%zu affine-bytes=%zu\n",
                        compiled.embedding_metadata.size(), compiled.embedding_payload.size(),
                        compiled.embedding_affine.size());
        }
