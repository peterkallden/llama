...
void roundtrip(astc_vulkan_ise_range range, const std::vector<uint16_t> & input) {
    ...
    assert(astc_vulkan_ise_pack(range, input, packed));
    const auto layout = astc_vulkan_ise_evaluate(range, input.size());
    assert(layout.valid && packed.size() == (layout.total_bits + 7u) / 8u);
    assert(astc_vulkan_ise_unpack(range, input.size(), packed, unpacked));
...
int main() {
    ...
    assert(legal.legal && legal.used_bits == 128 && legal.remaining_bits == 0);
    const auto legal_choices = astc_vulkan_enumerate_bit_budget({32, 4, 40, 15, 3, 128});
    assert(!legal_choices.empty());
    for (const auto & choice : legal_choices) {
        assert(choice.legal && choice.used_bits <= 128u);
    }
    const auto rejected = astc_vulkan_evaluate_bit_budget({32, 4, 40, 31, 4, 128});
    assert(!rejected.legal && rejected.used_bits > rejected.fixed_block_bits + 96u);
    return 0;
}
