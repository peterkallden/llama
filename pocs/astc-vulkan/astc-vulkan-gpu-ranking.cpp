...
bool astc_vulkan_build_gpu_ranking_atlas(
    ...
    result = {};
    if (!is_paired_d2_footprint(footprint) || atlas_blocks_x == 0 || candidates.empty()) return false;

    uint64_t total_candidates = 0;
    for (const auto & pool : candidates) {
    ...
    for (uint32_t source_block = 0; source_block < candidates.size(); ++source_block) {
