...
bool astc_vulkan_build_gpu_d1_ranking_atlas(
        astc_vulkan_footprint footprint, astc_vulkan_d1_semantic_decoder decoder,
    ...
    return astc_vulkan_build_gpu_d1_ranking_atlas_range(footprint, decoder, atlas_blocks_x,
        candidates, 0, static_cast<uint32_t>(candidates.size()), result);
}
...
bool astc_vulkan_build_gpu_d1_ranking_atlas_range(
        ...
        astc_vulkan_gpu_d1_ranking_atlas & result) {
    result = {};
    if (!astc_vulkan_footprint_is_valid(footprint) || atlas_blocks_x == 0 || candidates.empty() ||
    ...
    for (uint32_t source = first_source_block;
        ...
        if (candidates[source].empty()) return false;
        total_candidates += candidates[source].size();
    }
    const auto format = astc_vulkan_format(footprint);
    const uint64_t blocks_y = (total_candidates + atlas_blocks_x - 1) / atlas_blocks_x;
    if (total_candidates > std::numeric_limits<uint32_t>::max() ||
        blocks_y > std::numeric_limits<uint32_t>::max() ||
        atlas_blocks_x > std::numeric_limits<uint32_t>::max() / format.block_width ||
        blocks_y > std::numeric_limits<uint32_t>::max() / format.block_height) return false;

    result.footprint = footprint;
    result.decoder = decoder;
    result.atlas_blocks_x = atlas_blocks_x;
    result.atlas_blocks_y = static_cast<uint32_t>(blocks_y);
    result.width = atlas_blocks_x * format.block_width;
    result.height = result.atlas_blocks_y * format.block_height;
    result.payload.resize(static_cast<size_t>(total_candidates) * format.block_bytes);
    ...
    for (uint32_t source_block = first_source_block;
        ...
        for (uint32_t candidate_index = 0; candidate_index < pool.size(); ++candidate_index, ++record_index) {
            ...
            std::copy(pool[candidate_index].payload.begin(), pool[candidate_index].payload.end(),
                result.payload.begin() + static_cast<size_t>(record_index) * format.block_bytes);
            result.records.push_back({atlas_x, atlas_y, source_block, baseline_record, candidate_index});
        }
    }
    return true;
}
...
bool astc_vulkan_plan_gpu_d1_ranking_batches(
    ...
    for (uint32_t source = 0; source < candidates.size(); ++source) {
        ...
        if (pool_count == 0 || pool_count > max_candidate_count) return false;
    ...
    if (count != 0) batches.push_back({first, static_cast<uint32_t>(candidates.size()) - first, count});
