...
VkFormat astc_vulkan_vk_format(uint8_t footprint) {
    switch (footprint) {
        ...
        case 3: return VK_FORMAT_ASTC_8x6_UNORM_BLOCK;
...
bool astc_vulkan_texture::update_payload(
    ...
    const astc_vulkan_footprint fp = static_cast<astc_vulkan_footprint>(footprint);
    const uint64_t expected = astc_vulkan_image_bytes(fp, width_, height_);
    if (physical_device == VK_NULL_HANDLE || device == VK_NULL_HANDLE ||
        queue == VK_NULL_HANDLE || queue_family == UINT32_MAX || device_ != device ||
        resources_.image == VK_NULL_HANDLE || footprint_ != footprint || expected == 0 ||
        payload.size() != expected) {
        error = "ASTC Vulkan texture update dimensions or handles do not match";
        return false;
    }
    VkBuffer staging_buffer = VK_NULL_HANDLE;
    VkDeviceMemory staging_memory = VK_NULL_HANDLE;
    ...
    do {
        ...
        if (vkMapMemory(device, staging_memory, 0, payload.size(), 0, &mapped) != VK_SUCCESS) break;
        std::memcpy(mapped, payload.data(), payload.size());
        const VkMappedMemoryRange flush_range{VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE, nullptr,
            staging_memory, 0, VK_WHOLE_SIZE};
        if (vkFlushMappedMemoryRanges(device, 1, &flush_range) != VK_SUCCESS) {
            vkUnmapMemory(device, staging_memory);
            break;
        }
        ...
        const VkCommandBufferBeginInfo begin_info{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO, nullptr,
            VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, nullptr};
        if (vkBeginCommandBuffer(command_buffer, &begin_info) != VK_SUCCESS) break;
        const VkImageMemoryBarrier to_transfer{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, nullptr,
        ...
        vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
            VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &to_transfer);
        const VkBufferImageCopy copy{0, 0, 0, {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1},
            {0, 0, 0}, {width_, height_, 1}};
        vkCmdCopyBufferToImage(command_buffer, staging_buffer, resources_.image,
            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy);
        const VkImageMemoryBarrier to_shader{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, nullptr,
        ...
        if (vkQueueSubmit(queue, 1, &submit, fence) != VK_SUCCESS) break;
        submitted = true;
        success = vkWaitForFences(device, 1, &fence, VK_TRUE, UINT64_MAX) == VK_SUCCESS;
    } while (false);
    if (submitted && !success) vkDeviceWaitIdle(device);
    if (fence != VK_NULL_HANDLE) vkDestroyFence(device, fence, nullptr);
    if (command_pool != VK_NULL_HANDLE) vkDestroyCommandPool(device, command_pool, nullptr);
    ...
    if (staging_memory != VK_NULL_HANDLE) vkFreeMemory(device, staging_memory, nullptr);
    if (!success) {
        error = "ASTC Vulkan texture payload update failed";
        return false;
    }
    error.clear();
    return true;
