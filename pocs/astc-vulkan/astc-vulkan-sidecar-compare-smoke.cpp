...
bool run_case(case_data & data, astc_vulkan_footprint footprint,
    ...
    if (data.payload.empty() || data.decoded.size() != static_cast<size_t>(width) * height * 4 ||
        !read_metadata(data.metadata_path, data.reconstruction)) {
...
int main(int argc, char ** argv) {
    ...
    if (shader.empty() || activation_path.empty() || weights_path.empty() || width == 0 || height == 0 ||
        scalar.payload_path.empty() || scalar.decoded_path.empty() || scalar.metadata_path.empty() ||
        gauge.payload_path.empty() || gauge.decoded_path.empty() || gauge.metadata_path.empty()) return 2;
    ...
    if (!run_case(scalar, astc_vulkan_footprint::k6x6, spirv, trace, weights, width, height, error) ||
        !run_case(gauge, astc_vulkan_footprint::k6x6, spirv, trace, weights, width, height, error)) {
